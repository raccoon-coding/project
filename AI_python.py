...
import os
import numpy as np
Image_Path = './download/image_color.jpeg'
Image_Gray_Path = './download/image.jpeg'
...
def toYUV(rgb):
    ...
    return torch.from_numpy(np.asarray([Y, U, V]).reshape(3, 45, 80))
def toRGB(yuv, batchsize):
    ...
    for data in yuv:
        ...
        B = Y + 2.12798 * U
        lst.append([R,G,B])
    return np.asarray(lst).reshape(batchsize, 3, 45, 80)#.clip(0, 255)
...
def extractGray(batchSize, yuv):
    ...
    for data in yuv:
        lst.append(data[0])
    return np.asarray(lst).reshape(batchSize, 1, 45, 80)
...
class _netD(nn.Module):
    ...
    def forward(self, input):
        ...
        x = self.cnn(input)
        x = x.view(x.size(0), 512 * 2 * 5) # flatten it
        output = self.fc(x)
...
class _netG(nn.Module):
    ...
    def forward(self, input, noise_pure):
        ...
        noise = noise.view(noise.size(0), 1, 45, 80)
...
data = data.to(device)
batchSize = len(data)
gray_data = train_data.to(device)
gray = gray_data.cpu().numpy()
